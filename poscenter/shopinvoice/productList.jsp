...
product.goPage = function(pageNum,pageCount){
	var pageNum = '<input type="hidden" name="productPage.pageNum" value="'+pageNum+'" />';
	var pageCount = '<input type="hidden" name="productPage.pageCount" value="'+pageCount+'" />';
	$('#productQueryForm').append(pageNum);
	$('#productQueryForm').append(pageCount);
...
<div class="box-positon">
	<div class="rpos">当前位置: 店面进销存 - 商品列表</div>
	<div class="ropt">
	</div>
	<div class="clear"></div>
</div>
...
<form id="productQueryForm" style="padding-top:5px;" action="<%=request.getContextPath()%>/pos/shopInvoice!productList.do" method="post">
<div>
...
<tr>
	...
	<td align="center">
		<a href="<%=request.getContextPath()%>/pos/shopInvoice!shopInvoiceList.do?productId=<s:property value="#li.id"/>">查看</a>
	</td>
</tr>
</s:iterator>
</tbody>
